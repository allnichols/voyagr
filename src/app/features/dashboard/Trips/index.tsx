                        ...
                        <div className="card border-1 border-base-200" key={trip.id}>
                            <div className="card-body">
                                ...
                                <div className="mt-4 mb-4">
                                    ...
                                    <div className="mt-1">
                                        {trip.preferences.length === 0 && <span className="italic text-base-400">No preferences set</span>}
                                        {trip.preferences.map((pref) => (
                                            <span key={pref} className="badge badge-outline badge-secondary mr-1">{pref}</span>
                                        ))}
                                    </div>
                                </div>
                                <div className="card-actions justify-end">
                                    <button onClick={() => handleViewTrip(trip.id, trip.destination)} className="btn btn-sm btn-primary rounded-sm">View</button>
                                    <button onClick={() => removeTripMutation.mutate(trip.id)} className="btn btn-sm btn-outline btn-accent rounded-sm">Remove</button>
                                </div>
                            </div>
                        </div>
                    )
